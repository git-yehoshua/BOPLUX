...
- Architecture truth: server-authoritative; client requests via GDD §15 RemoteEvents; Impostor role state never replicates except to the Impostor's own client. Single experience, single place, single live server simulation; maps are versioned packages in `ServerStorage.Maps` cloned into `Workspace.MapRuntime.ActiveMap` per match (promoted 2026-09-06). The place is rebuilt 100% from repo scripts (2026-09-10) — repo code is the single source of truth.
- Safety and production boundaries: never trust client RemoteEvents; prototype-stage, no production posture.
- Current product direction: playable v1.0 per the GDD and Open Question Resolutions v1 + v2; all six §15 systems plus OQ-006 Sabotage, OQ-008 camping meter, and the 2026-09-10 EnvironmentSystem (Day/Night match mode rolled once per match + night flashlight with battery, L key; beam emits from a forward head mount) are implemented and unit-tested (75/75). **Place `BOPLUX.rbxl` rebuilt from the repo; map `Tulay_v004` is the active template (structural + procedural art only — the AssetLibrary dressing was removed owner-directed after persistent render glitches; library + v003 template deleted from the place, kit OBJs stay archived). HUD now has a real crosshair (dot + state ring: red target / amber jail / blue site), contextual interaction hints mirroring server ranges, and a red detonation-countdown timer with plant banners. Live verification: hint/color states, plant warning, red detonation timer, full plant→win route. Verification tier: unit tests + live single-client routes; 2-player-gated paths (incl. crosshair target state) still pending (REC-0004/0005). Remaining work: owner save + visual review, release-prep items.**
